...
function createPatchFunction (backend) {
  ...
  function assertNodeMatch (node, vnode, inVPre) {
    if (isDef(vnode.tag)) {
      ...
      return node.nodeType === (vnode.isComment ? 8 : 3)   // dom操作的nodeType， 8是注释节点， 3是文本节点
    }
  }
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly, parentElm, refElm) {
    ...
    if (isUndef(oldVnode)) {
      ...
      var isRealElement = isDef(oldVnode.nodeType);   // dom操作的nodeType
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        if (isRealElement) {
          ...
          if (oldVnode.nodeType === 1 && oldVnode.hasAttribute(SSR_ATTR)) {   // dom操作的nodeType
